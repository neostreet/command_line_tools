...
#define ARG_FILE      1
#define ARG_USER      2
#define ARG_PASSWORD  3
#define ARG_DATABASE  4
#define ARG_TABLE     5
#define NUM_ARGS      6
...
#define MAX_DESCRIPTION_LEN 101
...
static char login_to_mysql_failed[] = "login_to_mysql() error: %d";
static char couldnt_open[] = "couldn't open %s\n";

static char grab_col_failure[] =
"grab_col() failed for col %d of line %d: %d\n";

static int login_to_mysql(char *user,char *password,char *database);
...
int main(int argc,char **argv)
{
  ...
  retval = login_to_mysql(
    argv[curr_arg+ARG_USER],
    argv[curr_arg+ARG_PASSWORD],
    argv[curr_arg+ARG_DATABASE]);

  if (retval) {
    printf(login_to_mysql_failed,retval);
    return 3;
  }
  ...
  retval = mysql_query(mySql,balance_query_buf);
  ...
  for ( ; ; ) {
    ...
    if (!strlen(check_number)) {
      if (!strlen(addl_info)) {
        ...
        printf("insert into %s (trans_date,description,"
          "amount,balance,addl_info) values (\n",
          argv[curr_arg+ARG_TABLE]);
        printf("  '%s','%s',%lf,%lf,'%s');\n",trans_date2,description,
...
static int login_to_mysql(char *user,char *password,char *database)
{
  ...
  if (!mysql_real_connect(mySql,NULL,user,password,database,
    MYSQL_PORT,NULL,0)) {
    return 2;
  }
