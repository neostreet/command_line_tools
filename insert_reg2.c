...
#define MAX_CREDIT_DEBIT_LEN 5
static char credit_debit[MAX_CREDIT_DEBIT_LEN];

static char usage[] =
"usage: insert_reg2 (-debug) delim file database table\n";
...
int main(int argc,char **argv)
{
  ...
  for ( ; ; ) {
    ...
    if (!strncmp(line,"Posted Date",11))
      continue;
    ...
    if (!strcmp(credit_debit,"CR"))
      bDebit = FALSE;
    else if (!strcmp(credit_debit,"DR"))
      bDebit = TRUE;
    else {
      printf("line %d: credit_debit col must contain CR or DR\n",line_no);
      break;
    }
    ...
    for (n = 0; n < 2; n++)
      trans_date2[2+n] = trans_date[6+n];
